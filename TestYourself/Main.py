...
class MainWindow(QtWidgets.QMainWindow, Ui_Main):
    ...
    def show_results_cm(self, lst):
        ...
        print(greatcount > int((self.cur.execute(f"""SELECT Best FROM Scoreboard
        ...
        if self.current_user[2] not in (self.cur.execute("""SELECT nickname FROM Scoreboard""").fetchall())[0]:
            ...
            self.con.commit()
        elif self.current_user[2] in (self.cur.execute("""SELECT nickname FROM Scoreboard""").fetchall())[0]\
            and greatcount > int((self.cur.execute(f"""SELECT Best FROM Scoreboard
                                                WHERE nickname LIKE '{self.current_user[2]}'""").fetchall())[0][0]):
            pass
            up_best = self.cur.execute("""UPDATE Scoreboard SET Best = ?
                                             WHERE nickname = ?""", (greatcount, self.current_user[2]))
            self.con.commit()
    ...
    def show_rating(self):
        score = self.cur.execute(f"""SELECT nickname, Best from Scoreboard
                                    ORDER BY Best DESC""")
        top_ten = score.fetchmany(10)
        place = 1
